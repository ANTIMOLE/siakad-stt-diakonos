...
const paketKRSSchema = z.object({
  ...
  semesterId: z.number().min(1, 'Pilih semester akademik'), // ✅ ADDED
...
export default function BuatPaketKRSPage() {
  ...
  const semesterId = watch('semesterId'); // ✅ ADDED
  ...
  useEffect(() => {
    if (angkatan && prodiId && semesterPaket && semesterId) {
      const prodiName = prodiId === 3 ? 'PAK' : 'TEO';
  ...
  const onSubmit = async (data: PaketKRSFormData) => {
    ...
    try {
      ...
      const payload = {
        ...
        semesterId: data.semesterId, // ✅ ADDED
        semesterPaket: data.semesterPaket,
        totalSKS: totalSKS,
        kelasMKIds: data.selectedKelasMKIds,
      };

      const response = await paketKRSAPI.create(payload);

      if (response.success) {
        toast.success('Paket KRS berhasil dibuat');
        ...
        if (response.errors) {
          ...
          toast.error(response.message || 'Gagal membuat paket KRS');
        }
      }
    } catch (err: any) {
      console.error('Create paket KRS error:', err);

      let errorMessage = 'Terjadi kesalahan saat membuat paket KRS';
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Buat Paket KRS"
        description="Buat paket KRS baru untuk angkatan"
        breadcrumbs={[
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                {/* Nama Paket (Auto-generated, Editable) */}
                  ...
                  <div className="grid gap-2">
                    ...
                    >
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="2">TEO</SelectItem>
                      </SelectContent>
                    </Select>
                    {errors.prodiId && (
