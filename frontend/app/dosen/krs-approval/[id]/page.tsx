        ...
        if (krsData.mahasiswaId) {
          try {
            ...
            if (khsResponse.success && khsResponse.data && khsResponse.data.length > 0) {
              // Get the most recent KHS (first item, assuming sorted by newest first)
              setLastKHS(khsResponse.data[0]);
            }
          } catch (khsError) {
            // KHS optional, don't fail if not found
            ...
            <div className="grid gap-4 md:grid-cols-4">
              <div>
                ...
                <p className="text-2xl font-bold">
                  {lastKHS.ips ? lastKHS.ips.toFixed(2) : '-'}
                </p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground">IPK</p>
                <p className="text-2xl font-bold">
                  {lastKHS.ipk ? lastKHS.ipk.toFixed(2) : '-'}
                </p>
              </div>
              <div>
